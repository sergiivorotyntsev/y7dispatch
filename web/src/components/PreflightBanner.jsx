...
function PreflightBanner({ runId, onIssueClick, mode = 'training', warehouseId = null }) {
  ...
  useEffect(() => {
    ...
    const fetchPreflight = async () => {
      setLoading(true)
      try {
        const data = await api.getRunPreflight(runId, mode, warehouseId)
        setPreflight(data)
      } catch (err) {
