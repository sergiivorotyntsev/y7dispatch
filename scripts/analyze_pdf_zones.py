...
def analyze_pdf():
    with pdfplumber.open(PDF_PATH) as pdf:
        ...
        for name, pos in sorted(header_positions.items(), key=lambda x: x[1]["x0"]):
            ...
            print(f"   {name}")
            print(
                f"      X: {pos['x0']:.1f} - {pos['x1']:.1f} pts  ({pct_x0:.1f}% - {pct_x1:.1f}%)"
            )
        ...
        header_row_words_sorted = sorted(
            header_row_words, key=lambda w: (round(w["top"] / 5) * 5, w["x0"])
        )
        ...
        if member_x and physical_x and seller_x:
            ...
            print(
                f"      Center column (ADDRESS):  X = {left_boundary:.1f} to {right_boundary:.1f} pts"
            )
            ...
            print(
                f"      Center column:  {(left_boundary / width) * 100:.1f}% to {(right_boundary / width) * 100:.1f}%"
            )
